...
def preprocess_data(df, target_col="loan_approval", sensitive_col=None):
    ...
    if sensitive_col and sensitive_col in df.columns:
        ...
        # Reshape for imputer - Use .to_numpy() to avoid Arrow-backed reshape issues
    ...
    if len(cat_cols) > 0:
        ...
        # Ensure categorical columns are object type to avoid Arrow issues during imputation
        X_cat_obj = X[cat_cols].astype(object)
        imputed_cat = cat_imputer.fit_transform(X_cat_obj)
